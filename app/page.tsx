  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {view === "feed" && <FeedView quests={appQuests.slice(0, 20)} people={JUDGES.slice(0, 8)} locationLabel={locationLabel} following={following} onFollow={toggleFollow} liked={likedPosts} bookmarked={bookmarkedPosts} onLike={(id) => toggleNumber(setLikedPosts, id, "detour:liked-posts")} onBookmark={(id) => toggleNumber(setBookmarkedPosts, id, "detour:bookmarked-posts")} onOpenQuest={openQuest} onSearch={() => setSearchOpen(true)} onFriends={() => setView("friends")} onLocate={requestLocation} onInvite={() => { setInviteCopied(false); setInviteOpen(true); }} />}
        {view === "explore" && (
          <>
...
function FeedView({ quests: items, people, locationLabel, following, onFollow, liked, bookmarked, onLike, onBookmark, onOpenQuest, onSearch, onFriends, onLocate, onInvite }: {
  quests: Quest[];
  people: Person[];
  ...
  return <section className="feed-view">
    <header className="feed-header"><div><p className="eyebrow">{new Intl.DateTimeFormat("en-US", { weekday: "long" }).format(new Date())} · {locationLabel}</p><h1>Find the good part.</h1></div><button className="feed-search" onClick={onSearch}><AppIcon name="search" /><span>Search quests, people, lists</span></button></header>
